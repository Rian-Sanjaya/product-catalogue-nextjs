      ...
      <div style={{ padding: '56px 20px' }}>
        ...
        <div>
          ...
          <div style={{ width: '100%', maxWidth: '1920px', margin: '0 auto' }}>
            <div className={`${styles.content_wrapper}`}>
              {
                techs && techs.slice(0, 4).map((product) => (
                  <div key={product.id} className={`${styles.content_card}`}>
                    <div className={`${styles.image_outer_frame}`}>
                      <div className={`${styles.image_inner_frame}`} onClick={() => router.push(`/${product?.category}/${product?.id}`)}>
                        <img srcSet={`/images/${product?.pic_default}`} alt={product?.name} className={`${styles.image_element}`} style={{ width: '100%', height: 'auto' }} />
                      </div>
                    </div>
                    <div style={{ fontSize: '16px', fontWeight: '300', marginTop: '20px' }}>
                      <div className={styles.text_ellipsis}>
                        <div>{product?.name}</div>
                      </div>
                      <div style={{ marginTop: '8px' }}>{`$${product?.price}`}</div>
                    </div>
                  </div>
                ))
              }
            </div>
            <div style={{ paddingTop: '20px', borderBottom: '1px solid #e5e5e5'}}></div>
          </div>
        </div>
        ...
        <div>
          <div className={`${styles.section_header}`}>
            <Link href={`/lifestyle`}>
              <a className={`${styles.title_box}`}>
                <h2 className={styles.title}>{firstWordsToUpperCase('lifestyle')}</h2>
              </a>
            </Link>
          </div>
          <div style={{ width: '100%', maxWidth: '1920px', margin: '0 auto' }}>
            <div className={styles.lifestyle_content_wrapper}>
              {
                lifestyles && lifestyles.slice(0, 2).map((product, idx) => (
                  <div key={product.id} className={`${styles.content_card}`}>
                    <div className={`${styles.image_outer_frame}`} style={{ backgroundColor: '#eef2f3' }}>
                      <div className={`${styles.image_inner_frame}`} onClick={() => router.push(`/${product?.category}/${product?.id}`)}>
                        <img srcSet={`/images/${product?.pic_default}`} alt={product?.name} className={`${styles.image_element}`} style={{ width: '100%', height: 'auto' }} />
                      </div>
                    </div>
